...
\begin{solution}
    \begin{qparts}
        \item This is true. Assume $f(x) = O(g(x))$. By the definition of big-O, there exists $c_1, k \in \mathbb R$ such that for all $x \geq k$, $f(x) \leq c_1g(x)$. If $f(x) \leq c_1 g(x)$, then \[2^{f(x)} \leq 2^{c_1g(x)} = 2^{c_1}2^{g(x)}\] So $2^{f(x)} = O(2^{g(x)})$ with $c = 2^{c_1}$ and $k = k$.
